...
class CapAnnDataDF(pd.DataFrame):
    ...
    def join(self, other: Any, on=None, how='left', lsuffix='', rsuffix='', sort=False):
        result = super().join(other, on=on, how=how, lsuffix=lsuffix, rsuffix=rsuffix, sort=sort)
        if isinstance(other, CapAnnDataDF):
            new_columns = [col for col in other.column_order if col not in self.column_order]
        else:
    ...
    def merge(self, right, how='inner', on=None, left_on=None, right_on=None,
        ...
        result = super().merge(right, how=how, on=on, left_on=left_on, right_on=right_on,
                               left_index=left_index, right_index=right_index, sort=sort,
                               suffixes=suffixes, copy=copy, indicator=indicator, validate=validate)
        if isinstance(right, CapAnnDataDF):
            new_columns = [col for col in right.column_order if col not in self.column_order]
        else:
